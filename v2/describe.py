...
SYSTEM = (
    ...
    "- `*_source_value` / `*_source_concept_id` columns conventionally hold "
    "the verbatim value or non-standard concept from the ORIGINATING source "
    "system, as opposed to the standardized/mapped column next to them — "
    "describe that contrast when the pairing is visible in the schema.\n"
    ...
    "column names, SQL identifiers, code values, units, and domain acronyms "
    "(FK, PK, OMOP, SNOMED, concept_id, ...) in their original form; the "
    "prose around them is Korean. Use declarative dictionary style (~다/~함)."
)
