...
flowchart LR
    ...
    I --> J
    D --> K["store (선택)<br/>MySQL 메타스토어<br/>설명·검수이력·온톨로지"]
    E --> H["웹앱: 런 목록 · 트리 카탈로그(편집) · 그래프 · text2sql"]
    G --> H
    J --> H
    K --> H
```

파이프라인은 **단일 프로세스 인메모리 체인**이다(`pipeline.py`). 각 단계는 dict를
반환하는 함수이고, 중간 산출물을 파일로 쓰지 않는다 — 다음 단계로 메모리에서 그대로
넘긴다. 최종 결과(카탈로그·설명·온톨로지)만 **MySQL 메타스토어**에 적재하고, 스키마
그래프는 Neptune에, 검색 인덱스는 OpenSearch에 적재한다. **MySQL이 단일 진실 공급원
(source of truth)이며 필수다** — 디스크의 `*.json` 산출물은 더 이상 만들지 않는다.

| 단계 | 함수 | 입력 → 출력(메모리 dict) |
|---|---|---|
| 1. 프로파일링 | `profiler.build_profile(conn)` | DB → profile (통계, 샘플, 후보키) |
| 2. 관계 복원 | `relations.recover_relations(profile, conn)` | profile → relations (PK/FK + 출처 + 신뢰도) |
| 3. 의미 추론 | `describe.build_descriptions(profile, relations, conn)` | → descriptions |
| 4. 카탈로그 | `catalog.build_catalog(profile, relations, desc)` | 1~3 통합 → catalog (원본주석 보존) |
| 5. 채점(eval) | `score.score_run(desc, relations)` | catalog vs 정답지 → judge/cosine/F1 (with_truth 런만) |
| 개념 레이어 | `concepts.extract_concepts(catalog)` | catalog → `(:Concept)` 온톨로지 |
| 적재 | `store.repo.upsert_run(...)` | catalog·설명·온톨로지 → **MySQL (필수)** |
| 7. 스키마 그래프 | `graph.load_catalog_to_graph(run_key, catalog)` | catalog → Neptune (런별 전용 그래프) |
| 메타 RAG | `metasearch.index_catalog(run_key, catalog)` | catalog → OpenSearch 벡터 인덱스 |
| text2sql | `text2sql.run_text2sql(q, rid)` | 질문 → RAG+그래프 → SQL 생성·실행 (LangGraph) |
| 웹앱 | `webapp.py` | 런 관리 + 새 DB 연결 + 카탈로그(편집)/그래프/text2sql (`:8200`) |

`pipeline.run_pipeline(run_key, ...)`가 위 전 단계를 한 프로세스에서 엮는다. 웹앱은
이를 백그라운드 스레드로 호출하고, 조회·편집·그래프·text2sql 모두 메타스토어에서 읽는다.
...
(고객 모드에선 ④ 원본 DB에 이미 있는 COMMENT도 단서로 읽는다 — 위 생성 규칙 참조.
평가/블라인드 모드에선 무시. 어느 경우든 **읽기 전용**: DB를 변경하지 않는다.)
...
- **키 컬럼은 무엇의 식별자인지까지**: "기본 키"가 아니라 "각 개인(환자)의 고유 식별자; PERSON
  테이블의 기본 키".
- **원본값 vs 정제값 대비** (도메인 중립): 같은 속성을 원본(자유텍스트/원본코드)과 표준화된
  값 두 컬럼으로 나눠 둔 게 이름·값 모양으로 보이면 그 대비를 설명한다. 특정 제품·표준을
  가정하지 않고 **주어진 이름과 데이터만으로** 판단한다. (초기엔 OMOP `*_source_value` 같은
  특화 힌트가 있었으나 평가 순수성을 위해 제거 — §2.3, §4.3 블라인드 검증)
- **기존 DB 주석 활용** (고객 모드): 원본 DB에 COMMENT가 있으면 `existing_comment`로 강한
  힌트로 받되, 데이터로 검증해 맹목 복사하지 않는다 (주석이 오래돼 틀릴 수 있음). 평가 모드
  (`V2_USE_COMMENTS=0`)에선 무시해 블라인드 유지.
...
물리 스키마 그래프 위에 비즈니스 개념 층을 얹는다. **런마다 전용 Neptune Analytics 그래프**에
openCypher로 적재된다 (런 간 물리적 분리 — 같은 테이블명이 여러 런에 있어도 섞이지 않음;
그래프 ID는 런의 `meta.json`에 저장).
...
- **카탈로그 = 메타스토어 레코드**: 컬럼 단위 구조화 레코드(설명·키·통계·증거·신뢰도)는
  MySQL에 행으로 저장된다. 웹 뷰어가 요청하는 `catalog.json`은 메타스토어에서 즉석
  재구성한 응답이지, 디스크 파일이 아니다 [P10의 Spider 2.0 교훈: 정적 블롭이 아니라
  검색 가능한 레코드로].
- **읽기 전용 원칙**: 원본 DB는 조회만 하고 변경하지 않는다. 원본 DB에 주석을 써넣는
  COMMENT/DDL SQL은 생성하지 않는다.
- **원본 주석 보존**: 원본 DB에 기존 COMMENT가 있으면 코드가 그대로(룰베이스, LLM 미개입)
  메타스토어에 보존하고(`cat_tables.original_comment`/`cat_columns.original_comment`),
  생성 설명과 나란히 표시한다 (§3.A에서 프로파일러가 수집).
- **사람 검수·편집**: 웹앱 카탈로그 화면에서 DB/테이블/컬럼 설명을 ✎ 수정 → 저장 가능.
  AI 원본은 `descriptions.ai_text`에 그대로 남고, 검수본은 `current_text`, 수정 여부는
  `edited` 플래그로 구분된다.
- **메타스토어(MySQL, 필수)**: 결과의 단일 진실 공급원이다 (`store/`). 분석은 인메모리로
  진행되고 최종 결과만 적재된다 — 설명은 `descriptions`(AI 원본 `ai_text` + 검수본
  `current_text`), 검수 이력은 `revisions`(누가·언제·이전→이후), 온톨로지는 `concepts`,
  text2sql 질의 이력은 `t2sql_history`에 들어간다. `METASTORE_*` 미설정 시 웹앱은
  503으로 응답하며 동작하지 않는다. 구성·DDL은 INSTALL §7.
...
- **검수 이력 메타스토어**: AI 원본·검수본·감사 이력을 MySQL에 보존 → **구현됨**(`store/`,
  §6.2). 다중 사용자·다중 머신 공유.
...
# CLI 전체 파이프라인 (인메모리 체인 → MySQL + Neptune + OpenSearch, 파일 0개)
#   PG* 환경변수가 대상 DB를 가리킨다. METASTORE_* 필수.
../.venv/bin/python run.py --run-key myrun --name "내 DB"   # 카탈로그 생성·적재
../.venv/bin/python run.py --run-key omop --with-truth      # OMOP eval (채점 포함)
../.venv/bin/python run.py --no-graph --no-index            # MySQL만 적재(그래프/인덱스 생략)

# 한글화 (1회성 / 정답지 CSV — 신규 런은 처음부터 한국어 생성)
../.venv/bin/python translate.py truth    # 정답 CSV → truth/*_ko.csv

# 그래프·개념·인덱스는 run.py가 자동 적재한다. 개별 재적재가 필요할 때만:
../.venv/bin/python -c "from store import repo as r; import graph as G; \
  rk='myrun'; cat=r.load_run(rk); \
  G.load_catalog_to_graph(rk, cat, r.run_graph_id(rk))"   # 그래프만 재적재
# ★ 정리: 웹앱에서 런 삭제(✕) → 전용 그래프 자동 삭제. 또는 graph.py delete

# text2sql (메타데이터 RAG + 그래프, 모두 메타스토어 기준)
../.venv/bin/python text2sql.py --rid myrun "환자별 투약 이력"   # CLI 파이프라인
../.venv/bin/python metasearch.py search "환자별 투약 이력"      # 벡터 검색 프로브
...
| `NEPTUNE_GRAPH_ID` | (레거시) 공유 그래프 강제용 — 보통 비움. 런별 전용 그래프 ID는 메타스토어 `runs.graph_id`에 자동 저장 | — |
| `AOSS_ENDPOINT` | OpenSearch Serverless 컬렉션 엔드포인트 | (create-collection 출력) |
| `METASTORE_HOST`/`PORT`/`DB`/`USER`/`PASSWORD` (또는 `METASTORE_URL`) | **메타스토어 MySQL (필수)** — 결과의 단일 진실 공급원 | — |
| `V2_OUT_DIR` | (CLI) run-key 유도용 디렉토리명만 사용 — 파일은 쓰지 않음 | `out/` |
| `V2_MAX_LLM_TOKENS` / `V2_MAX_LLM_CALLS` | 자원 가드레일 [P1] | 800k / 400 |

비용 참고: describe 단계 약 268k in / 54k out 토큰(검증 루프 포함, 37테이블 기준).
Neptune Analytics는 **런별 전용 그래프당** 16 m-NCU 시간당 $0.48 (런 N개 = N배) +
OpenSearch Serverless(최소 OCU 시간당 과금) — **안 쓰는 런은 웹앱에서 삭제(✕, 그래프 자동 삭제)
+ OpenSearch 컬렉션 삭제로 과금 중지**.
